  ...
  return (
    <div>
      <ModalCreateAds
        ...
        hide={() => setShowCreateAdsModal(false)}
      />
      <ModalReceive
        address={address}
...
export function AdsTransactions({ address }: { address: PublicKey }) {
            ...
            <table className="table border-4 rounded-lg border-separate border-base-300">
              ...
              <tbody>
                {items?.map((ad, index) => (
                  <tr key={index}>
                    {adFields.map(field => (
                      <td key={`${field}-${index}`}>{ad[field]}</td>
                    ))}
                  </tr>
                ))}
                {(query.data?.length ?? 0) > 5 && (
                  <tr>
                    <td colSpan={adFields.length} className="text-center">
                      <button
                        className="btn btn-xs btn-outline"
                        onClick={() => setShowAll(!showAll)}
                      >
...
function ModalCreateAds({
  ...
  const handleSubmit = () => {
    ...
    // For example, dispatch an action to create ads
    hide();
  };
  ...
  return (
    <AppModal
      ...
      show={show}
      title="Create Ads"
      submitDisabled={!username || !price || !availableForSale || !paymentMethods}
      submitLabel="Create Ads"
      submit={handleSubmit}
    >
      ...
      <input
        ...
        onChange={(e) => setAvailableForSale(e.target.value)}
      />
      <div>
        <input
          type="number"
          ...
          onChange={(e) => setAmountRange({ ...amountRange, from: e.target.value })}
        />
        <span> to </span>
        <input
          type="number"
          ...
          onChange={(e) => setAmountRange({ ...amountRange, to: e.target.value })}
        />
      </div>
      <input
        type="text"
